...
const RPC_CLOSE_FUNCTION = 'close_tournament_position';
...
async function placeTournamentOrder(params) {
    ...
    return await res.json(); // { order, position, portfolio }
}

/**
 * Закрытие позиции целиком:
 * params: { entry_id, symbol, executed_price }
 */
async function closeTournamentPosition(params) {
    const url = `${SUPABASE_URL}/rest/v1/rpc/${RPC_CLOSE_FUNCTION}`;
    ...
    if (!res.ok) {
        const text = await res.text();
        throw new Error(`Supabase RPC error (close): ${res.status} ${res.statusText} - ${text}`);
    }
